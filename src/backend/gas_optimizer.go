...
func (go *GasOptimizer) CalculateOptimalTip(ctx context.Context, urgency protocol.UrgencyLevel) (*big.Int, error) {
	suggestedTip, err := go.client.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
