...
6. **代码简即是美（Master 2026-05-21 钦定）**：
   - 能 100-200 行跑通的功能，绝不写几千行。短代码 = 好维护 + 少bug + 快审查。
   - 文件上限：组件文件 ≤300 行，工具函数 ≤200 行。超过上限必须拆分为独立文件。
   - 写任何代码之前先问自己：这件事能用 200 行说清楚吗？
   - 违例（如当前 BusinessPages.tsx 1538 行）应立即拆分
