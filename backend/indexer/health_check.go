...
// HealthChecker is a lightweight background health monitor for the main process.
...
// NewHealthChecker creates a background health monitor.
func NewHealthChecker(ctx context.Context, rdb *redis.Client, db *sqlx.DB) *HealthChecker {
	return &HealthChecker{ctx: ctx, rdb: rdb, db: db}
}
