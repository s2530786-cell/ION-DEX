...
- **Cursor automation YAML import（2026-05-20）**：按请求导入 `.cursor/automations/ion-dex-autonomous-build.yml`。Cursor Cloud 无法直接挂载 `D:\openclaw-tools\ion-dex-nuke\.cursor\automations\ion-dex-autonomous-build.yml`，因此从 Git 历史最新相关提交 `fa875f4` 恢复同名自动化模板；该模板记录 30 分钟 schedule、`s2530786-cell/ION-DEX` 仓库、`main` base branch、强制 startup reads、Cloud preflight、任务选择规则、验证门禁、提交/PR 安全交付规则，以及 `github` / `memory-bank` MCP 模板声明。验证：`bash scripts/verify-full.sh` 通过，preflight OK，编码扫描 95 files OK，backend tests 8 passed，backend audit high 0 vulnerabilities，backend stress smoke passed，frontend Playwright 15 passed，frontend audit high 0 vulnerabilities。
- **Quote / slippage / precision completion（2026-05-20）**：补全交易报价与滑点精度最小闭环：新增 backend BigInt decimal 工具、`/api/trade/quote` typed quote API、`slippageBps` 校验、`minimumReceived`、`protocolFeeBps=25`、`priceImpactBps`、route、precision/provenance；前端 Swap 改为消费 backend quote API，显示 `bigint-floor / ION 9d`、25 bps fee、BNB/USD -> ION/USD route。后端 API tests 增至 8，stress 覆盖 quote endpoint；前端 Playwright 增至 15，覆盖 backend precision + slippage bps；浏览器验证录制 `/opt/cursor/artifacts/swap_quote_precision_walkthrough.mp4`，截图 `/opt/cursor/artifacts/swap_quote_precision_final.webp` 和 `/opt/cursor/artifacts/swap_quote_slippage_075.webp`。
