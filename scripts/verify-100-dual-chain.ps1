...
$summary = Join-Path $env:TEMP "ion-verify-100-dual-chain-$stamp.txt"
$log = Join-Path $env:TEMP "ion-verify-100-dual-chain-$stamp.log"
...
"ION DEX 100-pass DUAL-CHAIN security audit" | Set-Content -Path $summary -Encoding utf8
...
for ($i = 1; $i -le $Iterations; $i++) {
  Write-Log ("PASS " + $i + "/" + $Iterations + " dual-chain-audit")
  $auditExit = Run-Step -Name "dual-chain-audit" -Command {
  ...
  if ($auditExit -eq 0) {
    ...
    $failed++
    $failureLine = "PASS " + $i + " FAILED dualChainAudit=" + $auditExit
...
if ($failed -eq 0 -and $passed -eq $Iterations) {
  ...
  Write-Log "RESULT=GREEN (100 dual-chain audit passes)"
  exit 0
}
