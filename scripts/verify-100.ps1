...
for ($i = $StartAt; $i -le $Iterations; $i++) {
  ...
  if ($backendVerifyExit -eq 0) {
    ...
    $backendStressExit = Run-Step "backend-stress" {
      # forge test (contracts dir) 閳?pass if "36 tests passed" found
      ...
      try {
        ...
        if ($stressResult -match "71 tests passed, 0 failed") { $global:LASTEXITCODE = 0; return }
        Write-Host "forge test FAILED"
        throw "forge test did not report 71 tests passed, 0 failed"
