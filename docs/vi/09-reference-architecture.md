**Languages:** [English](../09-reference-architecture.md) | [简体中文](../zh-CN/09-reference-architecture.md) | [繁體中文](../zh-TW/09-reference-architecture.md) | [Русский](../ru/09-reference-architecture.md) | [Español](../es/09-reference-architecture.md) | [Português](../pt/09-reference-architecture.md) | [العربية](../ar/09-reference-architecture.md) | [Français](../fr/09-reference-architecture.md) | [Deutsch](../de/09-reference-architecture.md) | [日本語](../ja/09-reference-architecture.md) | [한국어](../ko/09-reference-architecture.md) | [हिन्दी](../hi/09-reference-architecture.md) | [Türkçe](../tr/09-reference-architecture.md) | [Italiano](../it/09-reference-architecture.md) | [Bahasa Indonesia](../id/09-reference-architecture.md) | [Tiếng Việt](./09-reference-architecture.md) | [ไทย](../th/09-reference-architecture.md) | [Polski](../pl/09-reference-architecture.md)

# Reference Architecture And External Patterns

This document maps selected external repositories to ION DEX architecture decisions. It is a reference index, not a vendoring plan. Do not copy code or add dependencies from these repositories without a separate license, security, and maintenance review.


## Bắt đầu từ đây

- [Reference Architecture And External Patterns (English)](../09-reference-architecture.md)
- [Technical Architecture](./03-technical-architecture.md)
- [Trung tâm tài liệu](./index.md)
- [Chỉ mục whitepaper](./whitepaper-index.md)

## Key Sections

- Adoption Rules
- Reference Pool
- Phase 3 Backend Foundation
- Gateway Shape
- Initial API Surface
- Reliability Rules
- Suggested Tech Path
- Blockchain Reference Usage
- AI Agent And Sentinel Usage
- Advanced UI And Media Usage

## Đọc tiếp theo

- [Development Roadmap](./04-development-roadmap.md)
- [Security Audit And Stress Sandbox](./23-security-audit-and-stress-sandbox.md)
- [Ice Open Network Official Ecosystem Panorama](./ion-official-ecosystem-panorama.md)

> Ghi chú: nhánh ngôn ngữ này cung cấp một lối vào đọc công khai ổn định; các tài liệu công khai tiếng Anh vẫn là nguồn canonical cuối cùng.

