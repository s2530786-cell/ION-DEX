...
- **Cursor Automation YAML import（2026-05-20）**：新增 `.cursor/automations/ion-dex-autonomous-build.yml`，用于承载用户要求导入的 `D:\openclaw-tools\ion-dex-nuke\.cursor\automations\ion-dex-autonomous-build.yml`。当前 Cloud Agent 环境无法挂载 Windows `D:\` 路径，已从 Git 历史中的自动化模板恢复并按当前仓库规则适配：读取 AGENTS/SESSION/memory-bank，先运行 `node scripts/dev-preflight.mjs`，代码改动后运行 `bash scripts/verify-full.sh`，只提交当前任务相关文件，禁止 secrets、破坏性 git 和跳过验证。`docs/08-ci-agent-automation.md` 已记录该 YAML 作为 Cursor Automations UI 的 source-of-truth import template。验证：`python3` + PyYAML 解析 `YAML_OK`；`bash scripts/check-encoding.sh` 扫描 93 files OK；`node scripts/dev-preflight.mjs` OK；`bash scripts/verify-full.sh` OK（backend tests 6 passed、backend audit 0 vulnerabilities、stress smoke passed、frontend Playwright 14 passed、frontend audit 0 vulnerabilities）。
