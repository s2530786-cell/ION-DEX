...
- **Critical correctness automation fix（2026-05-20）**：PR #2 审查发现 CI 与本地 full verify 脱节：`scripts/dev-preflight.mjs` 已接入 `verify-full.*`，但 `.github/workflows/ion-dex-verify.yml` 未执行 preflight，可能导致 memory/security/design 必需文件损坏时本地红灯而 CI 绿灯。已在 GitHub Actions 的 Node setup 后、encoding 前增加 `node scripts/dev-preflight.mjs`。验证：先因本机依赖未安装出现 `tsc: not found`，执行 `npm ci --prefix backend && npm ci --prefix frontend` 后，`bash scripts/verify-full.sh` 通过；preflight OK，encoding 92 files OK，backend 6 tests passed，backend audit 0 vulnerabilities，backend stress passed，frontend build + Playwright 14 passed，frontend audit 0 vulnerabilities。
