...
export default function ShareMenu({ cardRef, text, filename, testIdPrefix = "share" }) {
  ...
  const onNative = () => guard(async () => {
    const ok = await nativeShare(cardRef.current, text, filename);
  ...
  const onCopy = () => guard(async () => {
    ...
    toast[ok ? "success" : "error"](ok ? "文案已复制" : "复制失败");
  });
  ...
  return (
    <div className="grid grid-cols-2 gap-2" data-testid={`${testIdPrefix}-actions`}>
      ...
      <Btn icon={Twitter} color="#1DA1F2" onClick={() => shareToX(text)} testid={`${testIdPrefix}-x`}>分享到 X</Btn>
      <Btn icon={Send} color="#229ED9" onClick={() => shareToTelegram(text)} testid={`${testIdPrefix}-tg`}>Telegram</Btn>
