...
- **Quote / slippage / precision completion（2026-05-20）**：补全交易报价与滑点精度最小闭环：新增 backend BigInt decimal 工具、`/api/trade/quote` typed quote API、`slippageBps` 校验、`minimumReceived`、`protocolFeeBps=25`、`priceImpactBps`、route、precision/provenance；前端 Swap 改为消费 backend quote API，显示 `bigint-floor / ION 9d`、25 bps fee、BNB/USD -> ION/USD route。后端 API tests 增至 8，stress 覆盖 quote endpoint；前端 Playwright 增至 15，覆盖 backend precision + slippage bps；浏览器验证录制 `/opt/cursor/artifacts/swap_quote_precision_walkthrough.mp4`，截图 `/opt/cursor/artifacts/swap_quote_precision_final.webp` 和 `/opt/cursor/artifacts/swap_quote_slippage_075.webp`。修复 E2E preview 子进程残留问题后，100-pass 门禁完成：`PASSED=100`，`FAILED=0`，`RESULT=GREEN`，摘要保存为 `/opt/cursor/artifacts/swap_quote_precision_verify_100_summary.txt`。
