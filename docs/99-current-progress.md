...
- **UI 视觉自检报告（2026-05-21）**：对照用户成品参考图与 `docs/10-ui-design-route.md` 完成差距审计，结论为 **工程 verify 绿灯 ≠ 视觉门禁通过**。主要缺口：CSS 示意 K 线（未接 `lightweight-charts`）、Dashboard 硬编码盘口/深度、业务页 `NeonCard`/`GlassPanel` 混用、`ProfileHub` 未玻璃化、无 `tests/visual/baseline/`。完整报告：`docs/ui-visual-self-audit-2026-05-21.md`；参考图 PNG：`docs/ui-audit-screenshots/ref-*.png`。
