...
Market surface (P0, local-seed until indexer live):

- `GET /api/markets/candles?symbol=&interval=&limit=`
- `GET /api/markets/depth`
- `GET /api/markets/orderbook?symbol=`
- `GET /api/markets/swap-stats?pair=`
- Response `provenance.source` is `local-seed` for chart/orderbook/TVL seed data.
```
