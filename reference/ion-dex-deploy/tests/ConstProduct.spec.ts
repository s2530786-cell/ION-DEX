...
describe('Const Product', () => {
    ...
    describe('wip', () => {
        it('ION/LION 主网预演 — 建池+真实swap (零风险沙箱)', async () => {
            console.log('\n========== ION/LION DEX 沙箱预演 ==========');
            // 1) 部署 Router + 发 ION/LION 两个 jetton + 建初始池
            //    验证档：池子注入 ION 10000 / LION 10000（1:1 起步，贴近真实 LION≈ION）
            const ION_IN_POOL = toNano(10000);   // 10,000 ION
            const LION_IN_POOL = toNano(10000);  // 10,000 LION
            const setup = await setupDex({
                createPool: {
                    name1: 'ION',
                    name2: 'LION',
                    amount1: ION_IN_POOL,
                    amount2: LION_IN_POOL,
                }
            });
            const router = setup.router;
            const ion = setup.token1;
            const lion = setup.token2;
            const pool = setup.pool as SBCtrPool;
            expect(pool).toBeDefined();

            const pd0 = await pool.getPoolData();
            console.log(`Router 部署成功: ${router.address}`);
            console.log(`ION/LION 池建成: ${pool.address}`);
            ...
            // 2) 真实 swap：用 100 ION 换 LION
            const AMOUNT_IN = toNano(100); // 100 ION
            ...
            // 用 swap 返回的 sender LION 钱包算实际到手
            const lionWallet = res.senderWalletOut;
            const lionBal = await getWalletBalance(lionWallet);
            console.log(`\n--- 真实 swap: 100 ION -> LION ---`);
            ...
            console.log(`发起方 LION 钱包余额(含初始mint): ${lionBal}`);

            // 断言：一侧储备增加(收到ION)、另一侧减少(付出LION) —— 真实 AMM 行为
            const oneSideUp =
            ...
            console.log('========== 预演通过：ION 链上第一个 DEX 的建池+swap 机制成立 ==========\n');
        });
    });
