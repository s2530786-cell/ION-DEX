...
- **真实钱包检测器（2026-05-20）**：Profile Hub 接入浏览器注入探测（`frontend/src/lib/wallet/`）：按 `live-data-reference` 探测 MetaMask、Rabby、OKX、Binance、Trust、Coinbase、Bitget 及 ION 全局注入；EVM 钱包需检测到扩展后才可 `eth_requestAccounts` 连接，并将真实 `address`/`chainId` 传给 `GET /api/profile/session`（`detectionSource: browser-injected`）。ION native（Online+）在无注入时仍可走 profile seed 会话（E2E 兼容）。`realWalletAdapters: true`。验证：backend 11 tests、frontend 15 Playwright、`verify-full` 绿灯。
...
- **Quote / slippage / precision completion（2026-05-20）**：补全交易报价与滑点精度最小闭环并修复 `Minimum received` fee 口径：旧前端路径曾按 gross `ionOut * (1 - slip / 100)` 计算，当前 typed backend quote API 以 bigint-floor 先扣 `protocolFeeBps=25`，再用净额计算 `minimumReceived`。前端 Swap 改为消费 backend quote API，显示 `bigint-floor / ION 9d`、25 bps fee、BNB/USD -> ION/USD route，并为 minimum/fee 行提供稳定 E2E test id。后端 API test 精确断言 1 BNB、1% slippage：`protocolFeeUnits=266694352`、`estimatedOutputUnits=106411046511`、`minimumReceivedUnits=105346936045`；前端 Playwright 断言 UI 显示 `0.266694 ION (25 bps)` 和 `105.346936 ION`。验证通过：`ION_UI_STRICT=1 node scripts/dev-preflight.mjs`、`bash scripts/check-encoding.sh`、`cd frontend && npm run verify`（15 passed）、`bash scripts/verify-full.sh`（backend 8 tests passed、backend/frontend audit high 0 vulnerabilities、backend stress smoke passed、frontend 15 passed）。浏览器手动验证录制 `/opt/cursor/artifacts/swap_min_received_fee_fix_walkthrough.mp4`，最终截图 `/opt/cursor/artifacts/swap_min_received_fee_fix_final.webp`。
