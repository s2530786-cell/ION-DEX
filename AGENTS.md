...
- Both backend and frontend have pre-existing TypeScript type errors on `main`. The backend `tsc` still emits JavaScript despite errors (default behavior when `noEmitOnError` is not set), so the server and tests can run. The frontend `tsc && vite build` fails at the `tsc` step, but `npm run dev` (Vite dev server) works because Vite uses esbuild for transpilation and ignores type errors.
- In Cloud Agent VMs, `npx playwright install chromium` fails due to egress restrictions. Workaround: symlink the pre-installed system Chrome (`/usr/local/bin/google-chrome`) to Playwright's expected paths: `mkdir -p /home/ubuntu/.cache/ms-playwright/chromium_headless_shell-1223/chrome-headless-shell-linux64/ && ln -sf /usr/local/bin/google-chrome /home/ubuntu/.cache/ms-playwright/chromium_headless_shell-1223/chrome-headless-shell-linux64/chrome-headless-shell && mkdir -p /home/ubuntu/.cache/ms-playwright/chromium-1223/chrome-linux64/ && ln -sf /usr/local/bin/google-chrome /home/ubuntu/.cache/ms-playwright/chromium-1223/chrome-linux64/chrome`. The version numbers in the path (e.g., `1223`) must match the Playwright version installed; check `npx playwright --version` if the symlink path changes after a Playwright upgrade.
- The backend API serves mock data by default (no `CMC_API_KEY`). Endpoints like `/api/markets/tickers` and `/api/burn/summary` return errors without external API keys, but `/api/health`, `/api/tokens`, `/api/profile/demo`, `/api/bridge/routes`, and `/api/config/public` work with mock data.
